...
| 이름 | 값 | 필수 |
|---|---|---|
| `SMTP_HOST` | `smtp.hiworks.co.kr` (확인됨) | ✅ |
...
> **서버 주소는 `smtp.hiworks.co.kr` 로 확인했습니다** (DNS 조회 결과
> `smtp227s.hiworks.name` 의 별칭). 포트는 `465` 를 먼저 시도하고,
> 실패하면 `587` 로 바꿔보세요. 로그에서 바로 확인됩니다.
>
> 하이웍스 **메일 관리 → 메일 보안 → POP3/SMTP 설정**이
> '사용함' 이어야 하고, 해당 계정이 '예외 사용자' 에 없어야 합니다.
> (2026-08-21 확인: 사용함 / 예외 사용자 없음)
>
